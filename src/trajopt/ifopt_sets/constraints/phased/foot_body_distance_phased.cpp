#include "foot_body_distance_phased.hpp"

using namespace trajopt;

FootBodyDistancePhased::FootBodyDistancePhased(const SingleRigidBodyDynamicsModel& model, const std::shared_ptr<TrajectoryVars>& bodyPosVars, const std::shared_ptr<TrajectoryVars>& bodyRotVars, const std::shared_ptr<PhasedTrajectoryVars>& footPosVars, size_t numSamples, double sampleTime) : ConstraintSet(3 * numSamples, footPosVars->GetName() + "_foot_body_pos"), _model(model), _bodyPosVarsName(bodyPosVars->GetName()), _bodyRotVarsName(bodyRotVars->GetName()), _footPosVarsName(footPosVars->GetName()), _numSamples(numSamples), _sampleTime(sampleTime) {}

FootBodyDistancePhased::VectorXd FootBodyDistancePhased::GetValues() const
{
...
FootBodyDistancePhased::VecBound FootBodyDistancePhased::GetBounds() const
{
...
void FootBodyDistancePhased::FillJacobianBlock(std::string var_set, FootBodyDistancePhased::Jacobian& jac_block) const
{
    auto bodyPosVars = std::static_pointer_cast<TrajectoryVars>(GetVariables()->GetComponent(_bodyPosVarsName));
    auto bodyRotVars = std::static_pointer_cast<TrajectoryVars>(GetVariables()->GetComponent(_bodyRotVarsName));
    auto footPosVars = std::static_pointer_cast<PhasedTrajectoryVars>(GetVariables()->GetComponent(_footPosVarsName));
