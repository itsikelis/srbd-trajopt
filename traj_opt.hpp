...
namespace trajopt {
    ...
    public:
        ...
        void Init()
        {
            ...
            std::vector<double> phaseTimes; // Phase times for "Standing At Start" Phased Trajectories
            for (size_t i = 0; i < _args.numSteps - 1; ++i) {
                phaseTimes.push_back(_args.stepPhaseTime);
                phaseTimes.push_back(_args.swingPhaseTime);
            }
            phaseTimes.push_back(_args.stepPhaseTime); // Add last step (no swing after).

            // Calculate total time.
            double totalTime = std::accumulate(phaseTimes.begin(), phaseTimes.end(), 0.);
            ...
            ifopt::Component::VecBound posBounds = GetBoundsRegular(6 * _args.numKnots, "pos");
            auto posVars = std::make_shared<TrajectoryVars>(BODY_POS_TRAJECTORY, _args.numKnots, polyTimes, posBounds);
            _nlp.AddVariableSet(posVars);

            ifopt::Component::VecBound rotBounds = GetBoundsRegular(6 * _args.numKnots, "rot");
            auto rotVars = std::make_shared<TrajectoryVars>(BODY_ROT_TRAJECTORY, _args.numKnots, polyTimes, rotBounds);
            _nlp.AddVariableSet(rotVars);

            // Add Regular Constraint sets.
            auto dynamConstr = std::make_shared<DynamicsConstraint>(_model, _args.numSamples, _sampleTime);
            ...
            if (_args.gait == "jumping") {
                ...
                std::vector<double> pawPosPhasedTimes = GaitSequencer(phaseTimes, _args.numSteps, numSwings, _args.numKnotsPerSwing, true);
                std::vector<double> pawForcePhasedTimes = GaitSequencer(phaseTimes, _args.numSteps, numSwings, _args.numKnotsPerSwing, false);
                ...
                ifopt::Component::VecBound pawForceBounds(3 * numSwings + 6 * _args.numSteps * _args.numKnotsPerSwing, forceBounds);
                ...
                for (size_t i = 0; i < numSwings; ++i) {
                    pawForceBounds.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 0) = ifopt::BoundZero;
                    pawForceBounds.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 1) = ifopt::BoundZero;
                    pawForceBounds.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 2) = ifopt::BoundZero;
                }

                for (size_t i = 0; i < _model.numFeet; ++i) {
                    // Init foot #i
                    ifopt::Component::VecBound pawPosBounds(3 * _args.numSteps + 6 * numSwings * _args.numKnotsPerSwing, ifopt::NoBound);

                    pawPosBounds.at(0) = ifopt::Bounds(_model.feetPoses[i][0], _model.feetPoses[i][0]);
                    pawPosBounds.at(1) = ifopt::Bounds(_model.feetPoses[i][1], _model.feetPoses[i][1]);
                    pawPosBounds.at(2) = ifopt::Bounds(_terrain.z(_model.feetPoses[i][0], _model.feetPoses[i][0]), _terrain.z(_model.feetPoses[i][1], _model.feetPoses[i][1]));

                    auto pawPosVars = std::make_shared<PhasedTrajectoryVars>(PAW_POS + "_" + std::to_string(i), pawPosPhasedTimes, pawPosBounds, _args.numSteps, numSwings, _args.numKnotsPerSwing, true);
                    _nlp.AddVariableSet(pawPosVars);

                    _nlp.AddConstraintSet(std::make_shared<PhasedAccelerationConstraints>(pawPosVars));
                    _nlp.AddConstraintSet(std::make_shared<FootPosTerrainConstraints>(pawPosVars, _terrain, _args.numSteps, numSwings, _args.numKnotsPerSwing));
                    _nlp.AddConstraintSet(std::make_shared<FootBodyPosConstraints>(_model, posVars, rotVars, pawPosVars, _args.numSamples, _sampleTime));

                    // Add paw force var set.
                    auto pawForceVars = std::make_shared<PhasedTrajectoryVars>(PAW_FORCES + "_" + std::to_string(i), pawForcePhasedTimes, pawForceBounds, numSwings, _args.numSteps, _args.numKnotsPerSwing, false);
                    _nlp.AddVariableSet(pawForceVars);

                    _nlp.AddConstraintSet(std::make_shared<FrictionConeConstraints>(pawForceVars, _terrain, _args.numSamples, _sampleTime));
                ...
                // set.
                for (size_t i = 0; i < numSwings; ++i) {
                    pawForceBoundsEven.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 0) = ifopt::BoundZero;
                    pawForceBoundsEven.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 1) = ifopt::BoundZero;
                    pawForceBoundsEven.at((i + 1) * 6 * _args.numKnotsPerSwing + i * 3 + 2) = ifopt::BoundZero;
                }

                // Even foot indices.
                ifopt::Component::VecBound pawForceBoundsOdd(3 * _args.numSteps + 6 * numSwings * _args.numKnotsPerSwing, forceBounds);
                // When not in contact, force should be 0.
                for (size_t i = 0; i < _args.numSteps; ++i) {
                    pawForceBoundsOdd.at(i * 3 + i * 6 * _args.numKnotsPerSwing + 0) = ifopt::BoundZero;
                    pawForceBoundsOdd.at(i * 3 + i * 6 * _args.numKnotsPerSwing + 1) = ifopt::BoundZero;
                    pawForceBoundsOdd.at(i * 3 + i * 6 * _args.numKnotsPerSwing + 2) = ifopt::BoundZero;
                }
                ...
                for (size_t i = 0; i < _model.numFeet; ++i) {
                    if (i % 2 == 0) {
                        ...
                        auto pawPosVars = std::make_shared<PhasedTrajectoryVars>(PAW_POS + "_" + std::to_string(i), standGait, pawPosBounds, _args.numSteps, numSwings, _args.numKnotsPerSwing, true);
                        _nlp.AddVariableSet(pawPosVars);
                        ...
                        // Add paw force var set.
                        auto pawForceVars = std::make_shared<PhasedTrajectoryVars>(PAW_FORCES + "_" + std::to_string(i), swingGait, pawForceBoundsEven, numSwings, _args.numSteps, _args.numKnotsPerSwing, false);
                        _nlp.AddVariableSet(pawForceVars);

                        _nlp.AddConstraintSet(std::make_shared<FrictionConeConstraints>(pawForceVars, _terrain, _args.numSamples, _sampleTime));
                    }
                    else {
                        ifopt::Component::VecBound pawPosBounds(3 * numSwings + 6 * _args.numSteps * _args.numKnotsPerSwing, ifopt::NoBound);
                        ...
                        pawPosBounds.at(2) = ifopt::Bounds(_terrain.z(initFootPos[0], initFootPos[0]), _terrain.z(initFootPos[1], initFootPos[1]));

                        auto targetFootPos = _args.targetPos + _model.feetPoses[i];
                        pawPosBounds.at(pawPosBounds.size() - 3) = ifopt::Bounds(targetFootPos[0], targetFootPos[0]);
                        pawPosBounds.at(pawPosBounds.size() - 2) = ifopt::Bounds(targetFootPos[1], targetFootPos[1]);
                        pawPosBounds.at(pawPosBounds.size() - 1) = ifopt::Bounds(_terrain.z(targetFootPos[0], targetFootPos[0]), _terrain.z(targetFootPos[1], targetFootPos[1]));

                        auto pawPosVars = std::make_shared<PhasedTrajectoryVars>(PAW_POS + "_" + std::to_string(i), swingGait, pawPosBounds, numSwings, _args.numSteps, _args.numKnotsPerSwing, false);
                        _nlp.AddVariableSet(pawPosVars);

                        _nlp.AddConstraintSet(std::make_shared<PhasedAccelerationConstraints>(pawPosVars));
                        _nlp.AddConstraintSet(std::make_shared<FootPosTerrainConstraints>(pawPosVars, _terrain, numSwings, _args.numSteps, _args.numKnotsPerSwing));
                        _nlp.AddConstraintSet(std::make_shared<FootBodyPosConstraints>(_model, posVars, rotVars, pawPosVars, _args.numSamples, _sampleTime));

                        auto pawForceVars = std::make_shared<PhasedTrajectoryVars>(PAW_FORCES + "_" + std::to_string(i), standGait, pawForceBoundsOdd, _args.numSteps, numSwings, _args.numKnotsPerSwing, true);
                        _nlp.AddVariableSet(pawForceVars);
                ...
                if (_args.stepPhaseTime != _args.swingPhaseTime) {
                    std::cerr << "For trot, only equal swing and step times are supported at the moment!" << std::endl;
                    _args.swingPhaseTime = _args.stepPhaseTime;
                }
                ...
                // Generate gaits for even feet indices.
                std::vector<double> standGait = GaitSequencer(phaseTimes, _args.numSteps, numSwings, _args.numKnotsPerSwing, true);
                std::vector<double> swingGait = GaitSequencer(phaseTimes, _args.numSteps, numSwings, _args.numKnotsPerSwing, false);

                double max_force = 2. * _model.mass * std::abs(_model.gravity[2]);
                ifopt::Bounds forceBounds = ifopt::Bounds(-max_force, max_force);

                // Even foot indices.
                ifopt::Component::VecBound pawForceBoundsEven(3 * numSwings + 6 * _args.numSteps * _args.numKnotsPerSwing, forceBounds);
                // When not in contact, force should be 0.
                // numSwings in force var set is the number of step phases in foot pos var
                // set.
                for (size_t i = 0; i < numSwings; ++i) {
                ...
                if (_args.stepPhaseTime != _args.swingPhaseTime) {
                    std::cerr << "For bound, only equal swing and step times are supported at the moment!" << std::endl;
                    _args.swingPhaseTime = _args.stepPhaseTime;
                }
    ...
    protected:
            ...
            for (size_t k = 0; k < numPhases; k++) {
                ...
                if (isStance) {
                    ...
                    if (k > 0 && k < (numPhases - 1))
                        totalN += 1;
                    double swingSplineTime = phaseTime / static_cast<double>(totalN); // duration of each spline in a swing phase
        ...
        ifopt::Component::VecBound GetBoundsRegular(size_t size, std::string varSet)
        {
            ...
            if (varSet == "pos") {
                ...
                bounds.at(6 * _args.numKnots - 6) = ifopt::Bounds(_args.targetPos[0], _args.targetPos[0]);
                bounds.at(6 * _args.numKnots - 5) = ifopt::Bounds(_args.targetPos[1], _args.targetPos[1]);
                bounds.at(6 * _args.numKnots - 4) = ifopt::Bounds(_args.targetPos[2], _args.targetPos[2]);
                bounds.at(6 * _args.numKnots - 3) = ifopt::Bounds(_args.targetVel[0], _args.targetVel[0]);
                bounds.at(6 * _args.numKnots - 2) = ifopt::Bounds(_args.targetVel[1], _args.targetVel[1]);
                bounds.at(6 * _args.numKnots - 1) = ifopt::Bounds(_args.targetVel[2], _args.targetVel[2]);
            }
            else if (varSet == "rot") {
                ...
                bounds.at(6 * _args.numKnots - 6) = ifopt::Bounds(_args.targetRot[0], _args.targetRot[0]);
                bounds.at(6 * _args.numKnots - 5) = ifopt::Bounds(_args.targetRot[1], _args.targetRot[1]);
                bounds.at(6 * _args.numKnots - 4) = ifopt::Bounds(_args.targetRot[2], _args.targetRot[2]);
                bounds.at(6 * _args.numKnots - 3) = ifopt::Bounds(_args.targetAngVel[0], _args.targetAngVel[0]);
                bounds.at(6 * _args.numKnots - 2) = ifopt::Bounds(_args.targetAngVel[1], _args.targetAngVel[1]);
                bounds.at(6 * _args.numKnots - 1) = ifopt::Bounds(_args.targetAngVel[2], _args.targetAngVel[2]);
            }
